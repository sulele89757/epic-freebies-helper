...
这类项目无法承诺绝对 100% 成功率，因为 Epic 风控、共享云 IP、验证码题型和第三方模型响应都不可控。共享云 IP 风控明显时，可以可选配置 `BROWSER_PROXY`，支持 `http://用户名:密码@主机:端口`、`https://...`、`socks4://...` 和 `socks5://...`；未配置时网络路径不变。代理只能改变网络出口，不能保证绕过风控，其质量、可信度和费用由使用者负责。

代码层面的目标应该是：可恢复、可观测、不错报成功，并且每次失败都能留下足够证据支撑下一轮修复。
